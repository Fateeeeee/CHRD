...
class SegmentHead(nn.Module):
    # refer to bisenetv2
    def __init__(self, in_chan=256, mid_chan=256, n_classes=19):
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.iter=0
        self.cri_ce = nn.CrossEntropyLoss(ignore_index=args.ignore_label, reduce=True)   
        ...
        self.criterion_chrd = CHRDistiller(
            num_classes=train_dataset.num_class, temperature=2.0
        ).to(self.device)
        ...
        params_list.append(self.criterion_chrd)
        ...
        self.best_pred = torch.tensor(0.).to(self.device)
        self.topk = 5
        self.num_classes = 19
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            fdpd_loss = torch.tensor(0.).cuda()

            s_main, s_aux = s_outputs[0], s_outputs[1]
            t_main, t_aux = t_outputs[0], t_outputs[1]
            
            amcd_loss,amcd_loss2 = self.criterion_chrd(
                s_main, s_aux,
                t_main, t_aux
            )
            ...
            losses = (task_loss + amcd_loss)/(task_loss.detach()+amcd_loss.detach())+kd_loss/kd_loss.detach() + adv_G_loss + \
                        skd_loss + cwd_fea_loss + cwd_logit_loss +\
                        ifv_loss + at_loss + fitnet_loss +\
                        psd_loss + csd_loss+amcd_loss2/amcd_loss2.detach()
            ...
            amcd_loss_reduced = self.reduce_mean_tensor(amcd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    "Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || KD Loss: {:.4f} || AMCD Loss: {:.4f} " \
